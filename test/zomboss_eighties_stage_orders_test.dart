...
void main() {
  ...
  testWidgets('Eighties phase orders are localized and independently sortable', (
    tester,
  ) async {
    ...
    final animationCard = find.byKey(
      const ValueKey('zombossAnimOrderCard'),
    );
  ...
  testWidgets('adding and deleting an Eighties phase keeps all orders aligned', (
    tester,
  ) async {
    tester.view.devicePixelRatio = 1;
    tester.view.physicalSize = const Size(800, 1400);
    ...
    await tester.tap(
      find.byKey(const ValueKey('eightiesPhaseJamDropdown')),
    );
    await tester.pumpAndSettle();
    ...
    final phaseCard = find.ancestor(of: phaseSix, matching: find.byType(Card));
